...
def fetch_and_save_movie_details(movie: Movie, api_key: str):
    ...
    try:
        ...
        movie.genres.clear() # Clear existing genres to prevent duplicates/stale data
    ...
    try:
        ...
        movie.cast.clear() # Clear existing cast to prevent duplicates/stale data
        
        for member_data in cast_list:
            cast_name = member_data.get('name')
            if cast_name:
                cast_member, created = CastMember.objects.get_or_create(name=cast_name)
                movie.cast.add(cast_member)
        
        logger.info(f"Updated genres and cast for movie: {movie.title}")
...
@shared_task
@transaction.atomic # Ensures all DB operations (Movie/Recommendation/Genre/Cast) succeed or fail together
def fetch_and_save_recommendations(local_movie_pk):
    ...
    # recommendations_url = f"{TMDB_BASE_URL}{movie.third_party_id}/recommendations"
    ...
    try:
        ...
        logger.info(f"Querying TMDb for recommendations using external ID: {tmdb_movie_id}")
        ...
        if not recommended_movies_data:
            logger.info(f"No recommendations found for movie ID {tmdb_movie_id}.")
            return
        ...
        for movie_data in recommended_movies_data:
            ...
            recommended_movie, created = Movie.objects.update_or_create(
                ...
                defaults={
                    ...
                    'overview': movie_data.get('overview'), # Overview is sometimes available here
                }
            )
            
            # Fetch full details if the movie is new OR details are missing
            # This is important for getting runtime, genres, and cast
            if created or not recommended_movie.genres.exists(): 
                fetch_and_save_movie_details(recommended_movie, api_key)
            
            if created:
                logger.info(f"Saved new recommended movie: {recommended_movie.title}")
            ...
            logger.info(f"Created recommendation link: {source_movie.title} -> {recommended_movie.title}")
            
    except Movie.DoesNotExist:
        logger.error(f"Source movie with local PK {local_movie_pk} does not exist locally.")
    except requests.exceptions.RequestException as e:
